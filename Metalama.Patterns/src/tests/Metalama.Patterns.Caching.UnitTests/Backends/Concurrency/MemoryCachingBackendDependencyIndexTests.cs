...
public sealed class MemoryCachingBackendDependencyIndexTests
{
    ...
    /// registration, and the invalidation of its dependency does not remove it. <c>Clear</c> removes each key of the
    /// instance under the lock of that key, which the <c>SetItem</c> holds, so it cannot run inside the <c>SetItem</c>.
    /// </para>
    /// <para>The schedule is the following.</para>
    /// <list type="number">
    ...
    /// <c>MemoryCachingBackend.ClearCore:ClearRequested</c> pauses it before it processes the keys. This proves that
    /// <c>Clear</c> has been requested while the writer is paused.</description></item>
    /// <item><description>The test releases the clearer, then the writer. The clearer cannot acquire the lock of the key
    /// before the writer has stored its item and released that lock.</description></item>
    /// </list>
    /// <para>
