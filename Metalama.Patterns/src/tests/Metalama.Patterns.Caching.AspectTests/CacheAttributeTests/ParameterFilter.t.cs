...
internal class TheClass
{
  ...
  {
    if (_cacheRegistration_CachedMethod == null)
    {
      return this.CachedMethod_Source(x, y);
    }
  ...
  static TheClass()
  {
    _cacheRegistration_CachedMethod = CachedMethodMetadata.Register(typeof(TheClass).GetMethod("CachedMethod", BindingFlags.Public | BindingFlags.Instance, null, new[] { typeof(int), typeof(IDisposable) }, null) ?? throw new MissingMethodException("The method 'TheClass.CachedMethod(int, IDisposable)' could not be found using reflection."), new CachedMethodConfiguration() { AbsoluteExpiration = null, AutoReload = null, IgnoreThisParameter = null, Priority = null, ProfileName = (string? )null, SlidingExpiration = null }, false);
  }
