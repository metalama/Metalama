...
The latest variant defines `ROSLYN_5_10_0_OR_GREATER`, and five kinds of site use it. `INamedType.IsClosed` reads `ITypeSymbol.IsClosed`, which Roslyn 5.0 does not declare, so the read is compiled into the latest variant only and the property returns false in the other one; section 6 of `Metalama.Framework/docs/2027.0/DECISIONS.md` decides that mechanism for every C# 15 member. A site that emits such a member is compiled under the same condition, as `ModifierHelper.GetTypeSyntaxModifierList` is for `SyntaxKind.ClosedKeyword`, which the modifier list of an introduced closed class carries; the writer that requests the modifier, `INamedTypeBuilder.IsClosed`, carries the same condition and throws in the other variant, which is the variant that serves a host whose Roslyn predates C# 15, so that an aspect never silently obtains a class that is not closed. The unit tests of such a reader are compiled under the same condition, as `ClosedTypeTests` does for the facts that declare a closed class, because the source they compile needs a language version that only the latest variant parses. The `metalamaTests.json` of the `Tests/Aspects/CSharp15` folder requires the symbol for the same reason. And `UnknownAccessorInTemplate` and its `_Roslyn5_0` counterpart split one aspect test scenario in two, because Roslyn 5.0 reports `CS1014` on an empty span and Roslyn 5.10 reports it on the `setx` token. Name a new symbol after the Roslyn version at which the distinction appears, so that renumbering a variant does not rewrite the sites that use it.

A site that reads, emits or requests a member which the consumed Roslyn still marks with an experimental diagnostic names `ALLOW_PREVIEW_LANG_VERSION` in the same condition, so the emission in `ModifierHelper` and the setter of `INamedTypeBuilder.IsClosed` carry both symbols and the setter therefore throws in the default build of the latest variant as well. That symbol is the opt-in of `eng/RoslynPreview.props`, it ships disabled, and it is removed from a condition once Roslyn publishes the member without the marker.
