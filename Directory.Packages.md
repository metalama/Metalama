...
Both Roslyn values now agree with PB-2027.0. `RoslynApiMinVersion` is `5.0.0`, which is the Roslyn version that Rider presents and the lowest of any host in the baseline. `RoslynApiMaxVersion` is `5.10.0-1.26365.3`, which is the Roslyn version that `Metalama.Compiler` builds on. `MicrosoftBuildVersion` is `18.0.2`, the MSBuild of Visual Studio 2026 and of the .NET 10 SDK.
...
The user-surfacing packages follow the .NET line of the platform baseline, which is the .NET 10.0 line under PB-2027.0. Until issue #1903 they were pinned to the .NET 8.0 line. That pin protected a user application targeting `net8.0` from a forced transitive upgrade, and it was derived when `net8.0` was both a target framework of our own projects and a supported target framework for user applications. Issue #1876 removed both, so the configuration the pin protected left the tested set and the pin lost its justification.

Being user-surfacing is therefore no longer by itself a reason to hold a package on an older line. It remains a reason to raise a package deliberately rather than incidentally, because the version we declare is the version a user acquires transitively.

Two limits still bind, and each of them is measured per package rather than assumed. Neither follows from the audience of the package.
...
- A package that is also restored for `netstandard2.0` or `net472` must still publish an asset for those target frameworks at the higher version. The `Microsoft.Extensions.*` family and the Windows-specific `System.*` packages still publish both at the 10.0 line. The out-of-band family is where this limit actually bites; see *The Out-of-band family*.

A third limit applies to a package that the core flavour of the payload does not carry itself. The two flavours differ here: the core flavour deliberately does not embed `System.Text.Json`, because the parent assembly load context, which is Roslyn, provides it. Assembly binding rolls forward and never back, so a payload that references a line above the one its host provides fails to load. The floor is therefore the lowest version any host in the supported set provides, and `Microsoft.CodeAnalysis` 5.0, which is what the Rider backend presents, carries the 9.0 line. That is why `SystemTextJsonMinVersion` and the Roslyn 5.0 variant are on 9.0.10 while the Roslyn 5.10 variant is on 10.0.8.
...
### What #1903 changed, and the constraint it removed

The re-derivation moved every user-surfacing pin off the .NET 8.0 line. Three of them had been held by a constraint that the shipped artifact contradicts, and which is documented in the gotcha on separately-deployed downstream consumers below: `System.IO.Pipelines`, `Microsoft.Bcl.AsyncInterfaces` and `System.Threading.Tasks.Extensions` were treated as frozen by the central package management ceiling of the Metalama.Vsx versions already installed on user machines.

Metalama.Vsx merges `Metalama.Backstage`, `Metalama.Backstage.Tools` and `Metalama.Framework.DesignTime.Rpc` into a single `Metalama.Repacked` assembly with its own identity, so an installed extension carries its own copy of that code and our version choices do not reach it. An installed Metalama.Vsx 2026.1.5 holds 65 files, and none of those three packages is among them: they are resolved from the copies that Visual Studio itself provides, under the `devenv.exe.config` binding redirects, exactly like the out-of-band family. The premise of the freeze was therefore incorrect.

Two constraints replace it, and both are measurements rather than assumptions:

- The binding-redirect ceiling, which is assembly version 10.0.0.8 for `System.IO.Pipelines`, `System.Diagnostics.DiagnosticSource` and `Microsoft.Bcl.AsyncInterfaces`, and 4.2.4.0 for `System.Threading.Tasks.Extensions`. Those correspond to NuGet 10.0.8 and 4.6.3, with no headroom above either.
- Metalama.Vsx must still restore. It enables central transitive pinning, so its own pins must not be below what our packages require. They are raised in step, in `metalama/Metalama.Vsx`, and the two repositories have to move together.

Raising `System.Threading.Tasks.Extensions` to 4.6.3 also put it back on the same line as the rest of the out-of-band family, which must move together, and removed the `System.Text.Json` override that `Metalama.Patterns/Directory.Packages.props` carried to keep 4.6.3 out of the user-surfacing packages.

`System.Collections.Immutable` and `System.Reflection.Metadata` remain outside this rule: they are governed by the Roslyn version.
...
| `Roslyn.5.10.0.props` | 5.10+ | VS 2026 long-term servicing channel, VS 2027, the .NET 10 and .NET 11 SDKs, `Metalama.Compiler` |
...
Naming the variant `5.0.0` would therefore make `GetRoslynVersion` hand a Roslyn 5.5 host a payload that host cannot load. Naming it `5.10.0` makes the same host fall back to the `Roslyn.5.0.0` variant, which binds against `5.0.0.0` and loads correctly. The variant identity must never understate the Roslyn version that the payload binds against.
...
| VS 2026 long-term servicing channel, baseline of 2026-11 | about 5.11 | `Roslyn.5.10.0` |
| VS 2027 Stable | 5.10 or later | `Roslyn.5.10.0` |
| The .NET 10 and .NET 11 SDKs, and `Metalama.Compiler` | 5.0 and later | `Roslyn.5.10.0`, always the latest |
...
A symbol is warranted again only when the source has to branch on a distinction between two variants that no existing symbol expresses. Name it after the Roslyn version at which the distinction appears, not after a variant number, so that renumbering a variant does not rewrite the `#if` sites.
...
- **`Metalama.Vsx` constrains our pins at its build, not at its deployment.** This entry previously stated that a deployed extension re-resolves the flowed transitive dependencies of `Metalama.Framework.DesignTime.Rpc` on the user's machine, and that bumping any of them could break an installed extension. #1903 checked the shipped artifact and found otherwise, so the rule is restated here. What is true: `Metalama.Vsx` merges `Metalama.Backstage`, `Metalama.Backstage.Tools` and `Metalama.Framework.DesignTime.Rpc` into one `Metalama.Repacked` assembly carrying its own identity, so an installed extension holds its own copy of that code and our version choices do not reach it; and an installed Metalama.Vsx 2026.1.5 contains 65 files, among which `System.IO.Pipelines`, `System.Diagnostics.DiagnosticSource`, `Microsoft.Bcl.AsyncInterfaces` and `System.Threading.Tasks.Extensions` do not appear at all, because Visual Studio provides them under its own binding redirects. What remains true, and is the actual constraint: `Metalama.Vsx` enables `<CentralPackageTransitivePinningEnabled>true</CentralPackageTransitivePinningEnabled>`, so its own central pins must not sit below what our published packages require, or its **build** fails with NU1605. The two repositories therefore have to move together, and a change to any pin shared with `metalama/Metalama.Vsx` belongs in a pull request on each. Moving that traffic onto `Metalama.Framework.DesignTime.Contracts` (Guid-marked, COM type-equivalent — see `Metalama.Framework/docs/cross-process-communication.md` and metalama/Metalama#1605) would remove even that coupling.
