...
namespace Metalama.Framework.DesignTime
{
    ...
    {
        private static readonly string _generatedCodeHeader =
            DesignTimeSyntaxTreeHelper.GetHeader().NormalizeWhitespace().ToFullString();
        private static readonly WeakCache<Compilation, Compilation> _compilationsWithoutMetalamaGeneratedTreesCache = new();
        ...
        internal void AnalyzeSemanticModel( ISemanticModelAnalysisContext context )
        {
            ...
            try
            {
                ...
                // In VS Code, we're getting a compilation that includes syntax trees from our source generator.
                // So we need to remove them, before running the pipeline.

                bool IsMetalamaSourceGenerated( SyntaxTree tree )
                    => tree.ToString().StartsWith( _generatedCodeHeader, StringComparison.Ordinal );

                if ( compilation.SyntaxTrees.Any( IsMetalamaSourceGenerated ) )
                {
                    if ( IsMetalamaSourceGenerated( context.SemanticModel.SyntaxTree ) )
                    {
                        // This shouldn't happen (because we're setting GeneratedCodeAnalysisFlags.None), but handle the case to make sure anyway.

                        this._logger.Warning?.Log( $"TheDiagnosticAnalyzer.AnalyzeSemanticModel: the current syntax tree is generated by Metalama, so it's not going to be analyzed." );

                        return;
                    }

                    compilation = _compilationsWithoutMetalamaGeneratedTreesCache.GetOrAdd(
                        compilation,
                        c => compilation.RemoveSyntaxTrees( compilation.SyntaxTrees.Where( IsMetalamaSourceGenerated ) ) );
                }
...
#pragma warning disable RS1030 // Do not invoke Compilation.GetSemanticModel() method within a diagnostic analyzer, it is expensive.
                var semanticModel = compilation == context.SemanticModel.Compilation
                    ? context.SemanticModel
                    : compilation.GetSemanticModel( context.SemanticModel.SyntaxTree );
#pragma warning restore RS1030
                ...
                TemplatingCodeValidator.Validate(
                    ...
                    semanticModel,
                    ReportDiagnostic,
                    pipeline.MustReportPausedPipelineAsErrors && pipeline.IsCompileTimeSyntaxTreeOutdated( semanticModel.SyntaxTree.FilePath ),
                    true,
                    cancellationToken );
