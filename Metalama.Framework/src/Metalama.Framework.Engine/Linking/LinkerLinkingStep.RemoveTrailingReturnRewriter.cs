...
internal sealed partial class LinkerLinkingStep
{
    private sealed class RemoveTrailingReturnRewriter( SyntaxGenerationContext generationContext ) : SafeSyntaxRewriter
    {
        ...
        {
            if ( node.Statements.Count > 0 && node.Statements.Last().Kind() == SyntaxKind.ReturnStatement
                                           && node.Statements.Last() is ReturnStatementSyntax { Expression: null } )
            {
                ...
                {
                    if ( node.Statements[i].Kind() == SyntaxKind.ReturnStatement
                         && node.Statements[i] is ReturnStatementSyntax { Expression: null } returnStatement )
                    {
