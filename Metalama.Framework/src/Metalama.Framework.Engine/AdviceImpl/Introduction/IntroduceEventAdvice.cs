...
internal sealed class IntroduceEventAdvice : IntroduceMemberAdvice<IEvent, IEvent, EventBuilder>
{
    ...
    protected override IntroductionAdviceResult<IEvent> ImplementCore( EventBuilder builder, AdviceImplementationContext context )
    {
        ...
        // A field-like event holds a field, and the language does not permit one in a union declaration.
        if ( isEventField && !builder.IsStatic && targetDeclaration.IsUnion )
        {
