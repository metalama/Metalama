    ...
    where T : class, ICompilationElement
{
    ...
    public ResolvedAttributeRef GetAttributes()
    {
        switch ( this.TargetKind )
        {
            ...
            case RefTargetKind.PrimaryConstructor
                when this.GetSymbolIgnoringRefKind( this.CompilationContext ) is { Kind: SymbolKind.NamedType } and INamedTypeSymbol namedType:
    ...
    [Memo]
    private IDurableRef<T> DurableRef => this.RefFactory.DurableRefFactory.FromFullRef( this );

    public sealed override IDurableRef<T> ToDurable() => this.DurableRef;

    /// <inheritdoc cref="IFullRef.ToSerializableTypeId"/>
    /// <remarks>
    ...
    /// <see cref="SerializedDurableRefFactory.FromDeclarationOrType{T}"/>, so both routes to a durable reference agree.
    ...
    /// </remarks>
    public virtual SerializableTypeId? ToSerializableTypeId()
        => this.TargetKind == RefTargetKind.Default
           && this.GetSymbolIgnoringRefKind( this.CompilationContext ) is { Kind.IsType: true } and ITypeSymbol typeSymbol
            ? typeSymbol.GetSerializableTypeId( includeGenericContext: true )
            : null;
    ...
    private ISymbol ApplyRefKind( ISymbol symbol )
        => (this.TargetKind, symbol.Kind) switch
        {
            ...
            (RefTargetKind.Field, SymbolKind.Event) when symbol is IEventSymbol => throw new InvalidOperationException(
