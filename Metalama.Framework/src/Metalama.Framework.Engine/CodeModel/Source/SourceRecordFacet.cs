...
/// <summary>
/// The facet of a record read from source or from a referenced assembly.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// </remarks>
internal sealed class SourceRecordFacet : RecordFacet
{
    /// <summary>
    ...
    /// <summary>
    /// The full name of the parameter type of the <c>PrintMembers</c> method, which the signature is matched on.
    /// </summary>
    private const string _stringBuilderTypeFullName = "System.Text.StringBuilder";

    public SourceRecordFacet( INamedType type ) : base( type ) { }

    [Memo]
    public override IProperty? EqualityContractProperty
        => this.IsRecordClass ? this.Type.Properties.OfName( _equalityContractPropertyName ).SingleOrDefault() : null;

    [Memo]
    public override IMethod PrintMembersMethod => this.Type.Methods.OfName( _printMembersMethodName ).Single( IsPrintMembersMethod );

    [Memo]
    public override IMethod? CloneMethod => this.IsRecordClass ? this.GetCloneMethod() : null;

    [Memo]
    public override IConstructor? CopyConstructor
        => this.IsRecordClass
    ...
    [Memo]
    public override IMethod? DeconstructMethod => this.GetDeconstructMethod();

    [Memo]
    public override IReadOnlyList<IProperty> PositionalProperties => this.GetPositionalProperties();
