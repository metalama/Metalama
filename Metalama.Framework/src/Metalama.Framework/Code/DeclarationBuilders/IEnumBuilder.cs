...
/// <remarks>
/// <para>
/// This interface derives from <see cref="IMemberOrNamedTypeBuilder"/> and not from <see cref="INamedTypeBuilder"/>,
/// because an enum declares no member that an aspect can introduce and has neither a base type nor type parameters.
/// The operations that an enum does not have are therefore absent rather than present and failing.
/// </para>
/// <para>
...
/// <see cref="IMemberOrNamedTypeBuilder.IsPartial"/>. An enum is implicitly sealed, it may not be abstract or static,
/// and the language has no partial enum.
/// </para>
/// <para>
/// Every overload of <c>AddMember</c> applies the same three rules. The name must not be the name of a member already
/// added, and a duplicate throws an <see cref="ArgumentException"/>. The value is converted to
/// <see cref="UnderlyingType"/>, and a value that does not fit in that type throws an
...
public interface IEnumBuilder : IMemberOrNamedTypeBuilder
{
    ...
    /// <remarks>
    /// <para>
    /// The language allows one of <c>byte</c>, <c>sbyte</c>, <c>short</c>, <c>ushort</c>, <c>int</c>, <c>uint</c>,
    /// <c>long</c> and <c>ulong</c>. The setter throws an <see cref="ArgumentOutOfRangeException"/> for any other
    /// type.
    /// </para>
    /// <para>
    /// Setting this property after a member has been added throws an <see cref="InvalidOperationException"/>, because
    /// the value of a member is validated against the underlying type when the member is added.
    /// </para>
    /// </remarks>
    ...
    /// <remarks>
    /// <para>
    /// Setting this property to <c>true</c> adds the attribute, and setting it to <c>false</c> removes it. Adding the
    /// attribute through <see cref="IDeclarationBuilder.AddAttribute"/> has the same effect, and this property then
    /// reports <c>true</c>.
    /// </para>
    /// <para>
    /// The attribute changes how the value of the enum is formatted and parsed at run time, and it does not change the
    /// value of any member. An author who sets this property assigns the value of every member explicitly, because
    /// neither the language nor Metalama derives a power of two from the position of a member.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// Adds a member whose value is given as a <see cref="TypedConstant"/>, which is the form in which the value of a
    /// member of another enum is read.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The type of the constant must be an integral type or an enum, and any other type throws an
    /// <see cref="ArgumentException"/>. This overload therefore accepts the value of a member of another enum
    /// directly, which is how such a value is read from the code model.
    /// </para>
    /// <para>
