...
";
        ...
        /// <summary>
        /// Regression test for https://github.com/metalama/Metalama/issues/787.
        /// A method with compile-time parameters in an unannotated (run-time-only) type should be classified
        /// as a scope conflict, not silently treated as run-time-only.
        /// </summary>
        [Fact]
        public void MethodWithCompileTimeParametersInUnannotatedType()
        {
            const string code = @"
using Metalama.Framework.Code;
...
";
            ...
            using var testContext = this.CreateTestContext();
            var compilation = testContext.CreateCompilationModel( code );
            var type = compilation.Types.OfName( "AttributeExtensions" ).Single();
            var method = type.Methods.Single();
        ...
        /// <summary>
        /// Regression test for https://github.com/metalama/Metalama/issues/787.
        /// A method explicitly marked [CompileTime] in a run-time-only type should report a scope conflict error.
        /// </summary>
        [Fact]
        public void ExplicitCompileTimeMethodInRunTimeType()
        {
            const string code = @"
using Metalama.Framework.Aspects;
...
";
            ...
            // The method has an explicit [CompileTime] attribute, but is in a run-time-only type.
