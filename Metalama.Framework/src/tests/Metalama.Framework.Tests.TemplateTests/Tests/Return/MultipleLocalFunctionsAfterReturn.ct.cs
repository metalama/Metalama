...
namespace Metalama.Framework.Tests.TemplateTests.Return.MultipleLocalFunctionsAfterReturn
{
  [CompileTime]
  internal class Aspect
  {
    // Multiple local functions defined after a return should all be generated.
    private SyntaxNode __Template(ITemplateSyntaxFactory templateSyntaxFactory)
    {
      ...
      bool __skip1 = false;
      SyntaxToken TransformName = templateSyntaxFactory.GetUniqueIdentifier("Transform");
      SyntaxToken FormatName = templateSyntaxFactory.GetUniqueIdentifier("Format");
      if (meta.Target.Method.Name == "Method")
      {
        // return Transform( Format( meta.Proceed() ) );
      ...
      // object? Format( object? input ) { Console.WriteLine( "Format called" ); return input; }
      templateSyntaxFactory.AddStatement(__s1, SyntaxFactory.LocalFunctionStatement(default(SyntaxList<AttributeListSyntax>), default(SyntaxTokenList), SyntaxFactory.NullableType(SyntaxFactory.PredefinedType(SyntaxFactory.Token(SyntaxKind.ObjectKeyword)), SyntaxFactory.Token(SyntaxKind.QuestionToken)), templateSyntaxFactory.EscapeIdentifier(FormatName), null, SyntaxFactory.ParameterList(SyntaxFactory.SingletonSeparatedList<ParameterSyntax>(SyntaxFactory.Parameter(default(SyntaxList<AttributeListSyntax>), default(SyntaxTokenList), SyntaxFactory.NullableType(SyntaxFactory.PredefinedType(SyntaxFactory.Token(SyntaxKind.ObjectKeyword)), SyntaxFactory.Token(SyntaxKind.QuestionToken)), templateSyntaxFactory.EscapeIdentifier(inputName_1), null))), default(SyntaxList<TypeParameterConstraintClauseSyntax>), SyntaxFactory.Block(default, new Func<SyntaxList<StatementSyntax>>(delegate
      {
