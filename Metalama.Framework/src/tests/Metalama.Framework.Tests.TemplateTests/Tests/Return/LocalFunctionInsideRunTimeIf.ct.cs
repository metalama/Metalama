...
namespace Metalama.Framework.Tests.TemplateTests.Return.LocalFunctionInsideRunTimeIf
{
  [CompileTime]
  internal class Aspect
  {
    // Tests local function defined INSIDE a run-time if block (after compile-time return).
    // The local function should be generated because it's needed inside the run-time block.
