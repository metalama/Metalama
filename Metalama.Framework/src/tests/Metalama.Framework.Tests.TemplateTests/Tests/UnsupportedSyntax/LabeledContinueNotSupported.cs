...
// A labeled continue in a template is not supported, for the reason given in section 5 of
...
// accept a labeled continue. The project file therefore removes this file from the compilation. The test framework
// reads the file from the source directory, so the test still runs.
//
// The template also declares the label that the continue statement names, so the expected output reports the label too.

using Metalama.Framework.Aspects;
using Metalama.Framework.Engine.Templating;
using System;

namespace Metalama.Framework.Tests.AspectTests.Templating.UnsupportedSyntax.LabeledContinueNotSupported
{
    [CompileTime]
