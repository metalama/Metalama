...
public abstract class DesignTimePipelineTestsBase : UnitTestClass
{
    ...
    private static void DumpSyntaxTreeResult( SyntaxTree? syntaxTree, SyntaxTreePipelineResult syntaxTreeResult, StringBuilder stringBuilder )
    {
        ...
        foreach ( var durableDiagnostic in syntaxTreeResult.Diagnostics )
        {
            // Bound to the tree of the result, which is the tree the fallback below would have used anyway.
            var diagnostic = durableDiagnostic.ToDiagnostic( syntaxTree );
