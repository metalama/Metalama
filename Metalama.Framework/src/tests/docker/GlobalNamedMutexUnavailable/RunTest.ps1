<#
.SYNOPSIS
    Verifies that a build succeeds where the runtime cannot provide a machine-wide named mutex.
...
$arguments = @{
    ...
    Command = 'pwsh -NoProfile -NonInteractive -File Metalama.Framework/src/tests/docker/GlobalNamedMutexUnavailable/Container.ps1'
}
