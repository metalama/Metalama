...
- **Cross-version**: code compiled against one version of a Metalama assembly runs in the same CLR as code compiled against a different version of the same assembly. Examples: the user's project references Metalama 2026.1.x while the installed VS extension (`Metalama.Vsx`) was built against Metalama 2026.0.x. The 2026.0 line ships `Metalama.Framework.DesignTime.Contracts.dll` (v1); the 2026.1+ line ships `Metalama.Framework.DesignTime.Contracts.v2.dll` (v2, GUIDs rotated — see `Metalama.Framework.DesignTime.Contracts/GUID_ROTATION.md`). The v1 and v2 assemblies are designed to coexist in the same `devenv` process so both versions can satisfy their own consumers without interfering.
...
GUIDs are rotated when the assembly is bumped to a new physical name (v1 → v2). Within one major-line range (e.g. all 2026.1.x releases ship the v2 assembly with the v2 GUID set), GUIDs are frozen so CLR type-equivalence holds across all that line's deployments. Cross-line scenarios (e.g. the VSX shipping v1 against a user project on v2) fall back to the per-line entry-point manager: each line has its own AppDomain slot and named mutex (see `Metalama.Framework.DesignTime.Contracts/GUID_ROTATION.md`).
