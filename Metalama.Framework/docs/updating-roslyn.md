...
12. Do not add a `DefineConstants` entry to a variant props file unless the source has to branch on a distinction that no existing constant expresses. The variant props files currently define `ROSLYN_5_10_0_OR_GREATER`, which the aspect tests use and which also gates the engine sources that read a Roslyn member introduced with C# 15.
