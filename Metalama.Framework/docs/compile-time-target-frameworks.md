...
**A constraint on any solution.** The consumer cannot re-materialize a producer-copy object on its own. To
*serialize* an object, the binder must be able to name the copy its type comes from, and only the producing
project's closure can name its own copy — a consumer that tries to serialize a producer-copy object fails with
*"`ml!Contracts_…` is a compile-time assembly but it is not a part of the current project"*. So any conversion of
inherited artifacts to the consumer's copy has to involve the producer side; it is not something `D` can do
...
This is a **design-time** failure, because that is where two copies of the shared library's compile-time assembly
are loaded into a single `AssemblyLoadContext` (see point 5); at build time each project gets its own domain. It
is **not** specific to an IDE: it is observed in both Rider and Visual Studio, and reproduces with no IDE at all
...
- `Metalama.Framework.Tests.UnitTests/DesignTime/Pipeline/CrossTfmInheritedOptionsTests` — in-process regression
  test, no IDE required. It asserts the *absence* of the cast failure, so it fails while the bug is present.
- `src/tests/Standalone/Issue1710` — interactive reproduction: open the solution in Rider or Visual Studio.
  `dotnet build` succeeds by design (see point 5); the failure is design-time only.
...
- `src/tests/Standalone/Issue1710` — interactive reproduction (open in an IDE; `dotnet build` succeeds by design).
- `Metalama.Framework.Tests.UnitTests/DesignTime/Pipeline/CrossTfmInheritedOptionsTests` — in-process
  reproduction, no IDE required.
