### S-23. Metalama.Premium: union and closed architecture rule tests
...
from the union declaration to its case types is not attributed. S-18-6 attributes that reference in the core reference
index walker, and this story adds the tests that prove the result in `Metalama.Premium`.

#### Context

The tests must be compiled against a Roslyn that has the union syntax, so this story waits for S-14, which renames
the latest variant of `Metalama.Premium`. It is a separate story from S-18-6 because a pull request cannot span two
repositories.
