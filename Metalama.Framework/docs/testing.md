...
No production source branches on a variant constant, because both variants are Roslyn 5 and the engine treats them alike. The latest variant defines `ROSLYN_5_10_0_OR_GREATER`, which three aspect tests use where the expected output differs between Roslyn 5.0 and Roslyn 5.10. Unit-test projects deliberately do **not** override `LangVersion` per variant, so assertions can use the latest language in every variant. The process for adding a new Roslyn version is documented in [updating-roslyn.md](updating-roslyn.md).
