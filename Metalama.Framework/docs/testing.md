...
- `Metalama.Framework/Metalama.Framework.LatestRoslyn.slnf` (`IsTestOnly = true`) — this **solution filter is what actually runs the Framework tests**. It excludes the `.4.12.0` variant projects for speed, with one exception: `Metalama.Framework.Tests.AspectTests.4.12.0` is listed, so the aspect suite runs a second time against Roslyn 4.12.0 and with `MetalamaDurableRefKind=SerializableWithoutCache`. That exception exists for two reasons: the `.4.12.0` projects were being built on every run without a single one of their tests ever being executed, and an aspect test is a batch compilation, whose durable references hold the reference they were made from, so the identifier resolution path would otherwise have lost the coverage this suite used to give it. See issue [#1811](https://github.com/metalama/Metalama/issues/1811).
