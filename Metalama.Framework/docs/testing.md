...
- `Metalama.Framework/Metalama.Framework.LatestRoslyn.slnf` (`IsTestOnly = true`) — this **solution filter is what actually runs the Framework tests**. It includes only the projects built against the latest Roslyn version, with one exception: the aspect test suite is also listed in its variant built against the previous supported Roslyn version, which additionally sets `MetalamaDurableRefKind=SerializedWithoutCache`. There are two reasons for this exception. The variant projects were compiled by every run of the build, but none of their tests were executed. In addition, an aspect test is a batch compilation, and the durable references of a batch compilation store the reference they were created from, so the identifier resolution code would otherwise no longer be covered by this suite. See issue [#1811](https://github.com/metalama/Metalama/issues/1811).
