...
The variant props files define no preprocessor constant, because both variants are Roslyn 5 and the source treats them alike. A constant is added only when the source has to branch on a distinction between the variants. Unit-test projects deliberately do **not** override `LangVersion` per variant, so assertions can use the latest language in every variant. The process for adding a new Roslyn version is documented in [updating-roslyn.md](updating-roslyn.md).
