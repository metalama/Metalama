...
**CI** is TeamCity, generated into `.teamcity/settings.kts`; each build type (`DebugBuild`/`ReleaseBuild`/`PublicBuild`) simply runs `Build.ps1 … test` in a Docker image and publishes `artifacts/testResults/**`. The Docker-based tests are a sub-project of their own, with one configuration per platform (`DockerTestsLinuxX64`, `DockerTestsWindowsX64`) and a `RunAllDockerTests` composite over them. Each runs the generated `eng/RunDockerTests.ps1`, which discovers the tests under `Metalama.Framework/src/tests/docker`, selects the ones whose manifest lists the platform and reports each of them. Unlike the other configurations these run on the agent rather than in the product build container, because a test that needs a container of its own cannot be started from inside one. Coverage is handled inside the engineering `test` command (coverlet), gated by `SupportsTestCoverage` and the `eng/Coverage.props` a project imports; there is no separate dotCover step.
...
A scenario asserts on the diagnostics of the simulation itself through a **`designtime-test.json`** beside its solution, with the same `ExpectedDiagnosticsRegexes` / `ForbiddenDiagnosticsRegexes` / `ErrorRegexes` syntax as `test.json`. The two files are deliberately distinct and have one owner each: `test.json` belongs to the build engineering, which matches the **whole output** of the simulator process, including the trace and the output of the tools the analysis starts; `designtime-test.json` is read by the simulator and matches only the **diagnostics the simulation reported**. The same regular expression can hold for one and not the other, so a scenario that needs only the engineering verdict has no `designtime-test.json`. The reported diagnostics include those of the compilation itself, which is what lets a scenario detect an aspect that silently failed to apply: write code that binds only when the aspect applied, and forbid the compiler error that appears when it did not. Run one scenario directly with `dotnet Metalama.DesignTime.HostSimulator.dll <solution.sln>`; `--ignore-assertions` skips the file.
...
- `src/tests/docker/` — container end-to-end tests: `DockerTests.ps1` iterates subdirectories under a target (`win-x64`, `linux-x64`), each a `Dockerfile` + `test.ps1`, run via `DockerBuild.ps1` (optionally WSL). Scenarios include `CompilerLogs`, `MetalamaKill`, `NonStandardDotNetRoot`, `GlobalNamedMutexUnavailable`. Run only in the two docker CI configurations.
