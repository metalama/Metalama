...
- `Metalama.Framework/Metalama.Framework.LatestRoslyn.slnf` (`IsTestOnly = true`) — a solution filter for fast local builds. It includes only the projects built against the latest Roslyn version. The variants built against the older supported Roslyn versions are reached through the solution.
